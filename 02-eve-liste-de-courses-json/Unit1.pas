...
uses System.IOUtils, Unit3, Vcl.themes;
...
begin
  ...
  if (ch.Trim.Length > 0) then
    ajoutLigne(TJSONObject.Create.AddPair('libelle', ch.Trim).AddPair('coche',
      TJSONBool.Create(false)), true);
  displayListe;
end;
...
begin
  ...
  begin
    jso := TJSONObject.ParseJSONValue(tfile.ReadAllText(filename,
  ...
  try
    ...
    try
      ...
      begin
        ...
        begin
          l := c as tligne;
          jsa.AddElement(TJSONObject.Create.AddPair('libelle',
            l.CheckBox1.Caption).AddPair('coche',
            TJSONBool.Create(l.CheckBox1.Checked)));
        end;
      end;
      tfile.WriteAllText(TPath.Combine(TPath.GetDocumentsPath, FichName),
        jso.ToJSON, TEncoding.UTF8);
      listeModifiee := false;
    finally
      // FreeAndNil(jsa);
    end;
